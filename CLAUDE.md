...
> Andreas como aprovador. O WordPress fica só como rollback (issue #204, até ~25/08). 286 páginas
...
16. **Dizer que um ícone "tem o logo" sem medir a TINTA no tamanho em que ele é servido.** O favicon do site era a wordmark `MIROW & CO.` inteira, e a 32px ela tinha **0,00%** de pixel branco — um quadrado navy vazio, no ar, por anos. Ninguém mentiu: o arquivo *continha* o logo, tinha o navy certo e abria bem no visualizador. O que faltava era medir no **tamanho de render**: 11 caracteres em 32px dão ~3px cada, e o antialias come tudo. Quem descobriu foi o **Mario perguntando que símbolo era aquele** ao lado do nosso próprio site no Google. Mesma família dos erros 12 e 15 — asset conferido pela existência e pelo formato, nunca pelo pixel. **A regra: todo ícone se mede na largura em que a superfície o desenha** (16px aba, 24px resultado do Google, 48px mínimo do `.ico`, ~400px cartão de WhatsApp), e o veredito muda com o tamanho — a MESMA wordmark que é defeito no favicon está **correta** no `og-mirow.png` de 1200×630. Sentinela: `S171`.

17. **Verificação cujo ALCANCE é menor que o invariante que ela promete.** Não é o erro de medir a declaração em vez do efeito (P2.1) — é medir o efeito no **lugar errado**, ou num **recorte** do lugar. Quatro instâncias na onda 68, todas minhas, todas encontradas por cenário negativo e nenhuma pelo caso positivo:
    · **Só os nós de topo de uma árvore JSON.** `corrigir_logo` e depois a `S174` percorriam apenas os irmãos de `@graph`, e o Yoast escreve o `ImageObject` do logo **aninhado** dentro de `Organization.logo`. O script se declarou satisfeito tendo corrigido 3 de 109; a asserção ficou **verde com o arquivo do logo apagado**. Consertei no script e reproduzi na asserção meia hora depois. **Varredura de árvore é recursiva ou é ilusão.**
    · **A string em vez do valor.** A checagem do `143` procurava `/pt/#organization` enquanto 106 páginas usavam `https://mirow.com.br/pt/#organization`, escapado como `https:\/\/` no HTML. Re-parsear o JSON e olhar o valor resolve; procurar string no arquivo nunca resolve.
    · **"Um por página" não é "o mesmo em todas as páginas".** A `S174` cobrava `len(ids) > 1` por página, então uma página com um `@id` **diferente**, usado de forma consistente, passava. O invariante real era igualdade ao canônico.
    · **O atributo esconde o alcance.** O carimbo de cache só casava `href=` e `src=`, e o `msapplication-TileImage` declara o arquivo em `content=` — era o único dos 5 favicons saindo sem `?v=`, ou seja o bloco do Windows serviria o ícone velho para sempre. Quem pegou foi buscar cada ícone **com o navegador**, pela tag, como o visitante faz.
    **O padrão comum:** a asserção olhava um subconjunto e o título prometia o conjunto. É a mesma família da `V07` (que dizia "os números do hero" e media só `pt/` em 1920) e da `S57b` (que dizia "o logo segue o veículo" e comparava dois campos nossos entre si). **Ao escrever asserção, pergunte onde o dado pode estar que você não está olhando** — e exercite o cenário negativo, porque o positivo não distingue "passou" de "não mediu".

18. **Escrever em asserção um número que descreve o CONTEÚDO do site.** Quantas páginas, quantos líderes, quantas matérias, em que posição — é valor gêmeo do site, e divergem na primeira onda que muda o conteúdo. A onda 68 fechou com **6 asserções vermelhas e zero defeito de conteúdo**: as 3 páginas de um líder que saiu viraram stub, o total de páginas caiu de 109 para 106, e cinco marcadores mais a `S149` acusaram — a `S149` porque cobrava literalmente `!= 6` líderes. O comentário da própria tabela `MARCADORES` registrava que o número já havia sido corrigido **à mão nas ondas 29, 33 e 57**; a onda 68 seria a quarta. **O invariante nunca foi "109 páginas" — era "todas as páginas de conteúdo"**, e escrever o número era escrever a consequência em vez da causa. Agora os pisos são sentinelas (`TODAS`, `PT_EN`) resolvidas em runtime, e a contagem de líderes vem do `PAGINAS` do `110`, que é a mesma fonte de onde o JSON-LD, os cartões de preview e a meta description são montados. **E o título conta como asserção:** o da `S149` dizia "6 líderes" e virou "os líderes do cadastro" — mesma correção que a `S146` e a `S85` já tinham exigido. Antes de escrever um número numa asserção, pergunte de onde o código o derivaria.
