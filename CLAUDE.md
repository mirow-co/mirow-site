...
| **68** | **PRONTA, aguardando OK do Mario** (20/08) — **o favicon deixa de ser a wordmark e passa a ser a marca** (#246), e o mapa das **14 superfícies de ícone** que o site expõe (#247). Quem abriu o caso foi o Mario: *"que simbolo é esse que aparece do lado do nosso site no google??"* — o dono da marca não reconhecia o ícone do próprio site. **Medido, e o número é o achado:** o `…-32x32.png` tinha **0,00%** de pixel branco e o `…-192x192.png` **0,29%**, numa caixa de 125×**11** px. A wordmark `MIROW & CO.` tem 11 caracteres; a 32px cada um recebe ~3px e o antialias apaga tudo — o ícone servido era um **quadrado navy vazio**, que o Google ainda recorta em círculo. A marca do `LogoNeg.png` dá **10,67%**, 37× mais. Fonte copiada para `tools_onda6/dados/` (o script não referencia caminho de OneDrive — R14), navy conferido `(2,14,102)` = `#020E66`, alfa 255 em todo pixel (**conferido, não presumido** — erro 15). Os 4 PNG regerados **na mesma dimensão**, `.ico` agora **multi-frame 16+32+48** (48 é o mínimo do Google) e o **`/favicon.ico` da raiz passa a existir** — ele respondia **404**, e é o caminho em que navegador e crawler batem sem tag nenhuma. Os 5 favicons entraram no `27_cache_busting.py` (`v=82`): favicon é o caso mais agressivo de cache que existe. **A lição de classe:** favicon não aceita wordmark — o que cabe em 16px é UM glifo, e é aritmética de pixel por caractere, não gosto. O contra-exemplo mora no mesmo repo: o `og-mirow.png` (1200×630) É a wordmark e ali está **certo**, porque o cartão renderiza a ~400px; mesmo desenho, duas superfícies, dois vereditos. Suíte: **S171**, que mede a **tinta** de todo ícone declarado — lendo a lista do próprio HTML (padrão da S127) — e exige o frame de 48px; cobrar o NOME do arquivo passaria verde no dia em que alguém regenerasse os ícones da wordmark outra vez. **Os 3 cenários negativos foram exercitados e cada um acusa**, e foi um deles que pegou a S171 retornando `det` como LISTA: ela ficava **verde no caso bom** (com `det` vazio o caminho do detalhe nunca roda) e **quebrava o gate com `TypeError`** no primeiro caso ruim. O decodificador `_png_rgb` foi **promovido** de local (dentro da V39) a helper de módulo, em vez de duplicar 40 linhas — valor gêmeo. **O inventário (#247) achou 4 buracos abertos:** 6 páginas **sem `og:image` nenhuma** (as 3 de imprensa e as 3 de política — compartilhadas no WhatsApp saem sem imagem, e imprensa é a que se manda para jornalista), 6 páginas de líder usando retrato de **232×246** como cartão, `og:image` de artigo a 2560px e **927 KB**, e **duas `Organization` no JSON-LD** disputando o painel do Google — a do Yoast tem `logo` e nenhum endereço, a nossa da onda 59 tem endereço e **nenhum `logo`**. Ver `docs/ICONES-EXPOSTOS.md`. |
